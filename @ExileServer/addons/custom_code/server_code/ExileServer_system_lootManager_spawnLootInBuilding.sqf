...
];
{
	...
	if (_itemClassNames isEqualType "") then {_itemClassNames = [_itemClassNames];};
	{
		...
		if !(_itemClassName in _spawnedItemClassNames) then
		{
			...
			{
				if(getNumber(missionConfigFile >> "CfgExileLootSettings" >> "newYearGifts") isEqualTo 1) then
				{
					if ((floor (random 100)) < 50) then 
					{
						_lootHolder = createVehicle [selectRandom _presentClasses, _lootPosition, [], 0, "CAN_COLLIDE"];
					}
					else 
					{
