...
try 
{
	...
	_maximumBuildRightsPerTerritory = getNumber(missionConfigFile >> "CfgTerritories" >> "maximumMemberTerritoryPerPlayer");
	_numberMembers = _playerToBeAddedObject call AtlasClient_system_memberTerritory;
	if (_maximumBuildRightsPerTerritory <= _numberMembers) then 
	{
		throw format ["Limit reached for membership in the territories. Maximum %1 areas can be accessed.",_maximumBuildRightsPerTerritory];
	};
	...
	if((getNumber(missionConfigFile >> "CfgTerritories" >> "maximumMembersInTerritory")) <= (count _currentBuildRights)) throw "Maximum players in territory reached";
