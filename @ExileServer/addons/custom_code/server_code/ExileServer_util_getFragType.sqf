...
try 
{
	...
	_allowedCamp = getNumber(missionConfigFile >> "CfgTradeSafeZone" >> "penaltyNearSafeZone");
	if(_allowedCamp isEqualTo 1) then
	{
		if ((getPosATL _victim) call AtlasTradeAddons) then 
		{
			throw 8;
		};
	};
