...
if (_isLocked) then
{
	...
	{
		_vehicleOwner = _vehicleObject getVariable "ExileOwnerUID";
		_vehicleInTerritory = _position call ExileClient_util_world_getTerritoryAtPosition;
		if !(isNull _vehicleInTerritory) then
			{
				if ((_vehicleOwner) in (_vehicleInTerritory getVariable ["ExileTerritoryBuildRights", []])) then
				{
