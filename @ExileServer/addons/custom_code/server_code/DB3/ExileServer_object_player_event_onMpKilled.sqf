...
switch (_killType) do 
{
	...
	case 4:
	{
		...
		AtlasKillMessages = ["NPC", _pic, (name _victim), floor(_victim distance _killer), _txt, nil, nil];
		publicVariable "AtlasKillMessages";
	...
	case 5:
	{
		...
		AtlasKillMessages = [(name _killer), _pic, (name _victim), floor(_victim distance _killer), _txt, nil, nil];
		publicVariable "AtlasKillMessages";
	};
	case 6:
	{
	...
	{	
		_countDeath = true;
		_countKill = true;
		_systemChat = format ["%1 was killed by %2! Killed near safe-zone! Killer transfer 10% respect to victim!", name _victim, name _killingPlayer];
		
		_respectTransfer = round ((abs _oldKillerRespect) / 100 * (getNumber (configFile >> "CfgSettings" >> "Respect" >> "Percentages" >> "percentageTransferToVictim")));
		if (_respectTransfer < _minRespectTransfer) then
		{
			_respectTransfer = _minRespectTransfer;
		};
		_newVictimRespect = _oldVictimRespect + _respectTransfer;
		_newKillerRespect = _oldKillerRespect - _respectTransfer;
		_killSummary pushBack ["Dont kill near safe-zone!", -1 * _respectTransfer];		
...
if(_kUID isEqualTo '') then {_kUID = 'NPC';};
_kDistance = floor(_victim distance _killer);
if!(_kUID == 'NPC') then
{
	format["insertPlayerHistory:%1:%2:%3:%4:%5:%6:%7:%8", getPlayerUID _victim, name _victim,(mapGridPosition _killer),(mapGridPosition _victim),_kName,_kUID,_kDistance,_txt] call ExileServer_system_database_query_fireAndForget;
};
